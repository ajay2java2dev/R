...
create_ubcf_training <- function(input_data) {
  rec_UBCF = Recommender(data = input_data, method='UBCF', parameter = list (normalize = 'Z-score', method = 'Cosine', nn = 25))
  return (rec_UBCF)
}

create_ibcf_training <- function(input_data) {
  rec_IBCF = Recommender(data = input_data, method='IBCF', parameter = list (normalize = 'Z-score', method = 'Cosine', nn = 25))
  return (rec_IBCF)
}
  ...
  df <- eventReactive(input$btn, {
    withBusyIndicatorServer("btn", { # showing the busy indicator
      ...
      if(!is.null(user_ratings) & nrow(user_ratings) > 0) {
        ...
        user_input = data.frame(UserID=0, MovieID=movie_ids, Rating = movie_ratings, Timestamp = NA)
