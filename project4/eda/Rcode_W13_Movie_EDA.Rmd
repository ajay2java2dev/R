...
datatable(head(ratings, 6), class = "nowrap hover row-border", 
          options = list(dom = 't',
                         scrollX = FALSE, 
                         autoWidth = TRUE))
```
...
```{r, result='asis', echo=FALSE}
datatable(head(movies, 6), class = "nowrap hover row-border", 
          options = list(dom = 't',
                         scrollX = FALSE, 
                         autoWidth = TRUE))
```
...
movies_not_rated = movies %>% filter(!(MovieID %in% ratings$MovieID))
...
tmp = data.frame(Rating = 1:5, freq = as.vector(table(ratings$Rating)/nrow(ratings)))
...
stem(tmp$ratings_per_user, scale = 2)
...
```{r}
# SQL Equivalent - select MovieID, ratings_per_movie = count(1), ave_ratings = avg(rating) from ratings group by MovieID 
...
#unique(tmp[,1]) # list below using unique
...
# create a smaller list to work with
filtered_movies_genre <- movies %>% filter(grepl(input_genre, Genres))
head(filtered_movies_genre)
